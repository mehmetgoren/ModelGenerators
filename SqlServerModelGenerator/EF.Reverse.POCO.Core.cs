namespace SqlServerModelGenerator
{
    ...
    public class PocoCore
    {
       ...
       // var fileManager = EntityFrameworkTemplateFileManager.Create(this);
        ...
        Regex TableFilterExclude = new Regex(".*sysdiagram.*");
        Regex TableFilterInclude = null;


        public PocoCore(string connStr)
        {
            this.ConnectionString = connStr;
        }

        static readonly string[] ReservedKeywords = new string[]
        {
        ...
        public Tables LoadTables()
        {
            try
            {
                using (DbConnection conn = new SqlConnection())
                {
                    ...
                    var reader = new SqlServerSchemaReader(conn);
            ...
            catch (Exception)
            {
                ...
                //this.tw.WriteLine(string.Format("Failed to read database schema - {0}", error));
                //this.tw.WriteLine("");
                //this.tw.WriteLine("// -----------------------------------------------------------------------------------------");
                //this.tw.WriteLine("// Failed to read database schema - {0}", error);
                //this.tw.WriteLine("// -----------------------------------------------------------------------------------------");
                //this.tw.WriteLine("");
                return new Tables();
            }
        ...
        public sealed class Column
        {
            ...
            private void SetupEntity(bool includeComments)
            {
                ...
                comments = null;
                Entity = string.Format("public {0}{1} {2} {3}{4}", PropertyType, CheckNullable(this), PropertyNameHumanCase,
                    IsStoreGenerated ? "{ get; internal set; }" : "{ get; set; }", comments);
            }
            ...
            private void SetupConfig()
            {
                ...
                Config = string.Format("Property(x => x.{0}).HasColumnName(\"{1}\"){2}{3}{4}{5}{6};", PropertyNameHumanCase,
                    Name,
                ...
                switch (PropertyType.ToLower())
                {
                    ...
                    case "datetime":
                        ...
                        if (!DateTime.TryParse(Default, out dt))
                            //Default = Default.ToLower().Contains("getdate()") ? "System.DateTime.Now" : string.Empty; Burayı Değiştirdik.
                            Default = Default.ToLower().Contains("getdate()") ? "getdate()" : string.Empty;
                        else
                    ...
                    case "datetimeoffset":
                        ...
                        if (!DateTimeOffset.TryParse(Default, out dto))
                            Default = Default.ToLower().Contains("sysdatetimeoffset()")
        ...
        public static class Inflector
        {
            ...
            {
                return word;
                //return PluralizationService.Pluralize(word);//Burayı Değiştirdik.
            }
            ...
            {
                return word;
                //return PluralizationService.Singularize(word);//Burayı Değiştirdik.
            }
        ...
        private abstract class SchemaReader
        {
            ...
            protected SchemaReader(DbConnection connection)
            {
        ...
        private sealed class SqlServerSchemaReader : SchemaReader
        {
...
ORDER BY FK.TABLE_NAME,
        FK.COLUMN_NAME;";
            ...
            public SqlServerSchemaReader(DbConnection connection)
                : base(connection)
            {
            }
                ...
                bool includeComments)
            {
                        ...
                        if (lastTable != tableName || table == null)
                        {
                            ...
                            if (table == null)
                            {
                                table = new Table
                                {
                                    Name = tableName,
                                    Schema = schema,
                                    IsView = String.Compare(rdr["TableType"].ToString().Trim(), "View",
                                ...
                                table.NameHumanCase = (useCamelCase ? Inflector.ToTitleCase(singular) : singular)
                                    .Replace(" ", "").Replace("$", "");
                                // if((string.Compare(table.Schema, "dbo", StringComparison.OrdinalIgnoreCase) != 0) && prependSchemaName) Burayı Değiştirtik çünkü schema' yı yazıyordu.
                ...
                foreach (Column c in result.SelectMany(tbl => tbl.Columns.Where(c =>
                    tbl.Columns.FindAll(x => x.PropertyNameHumanCase == c.PropertyNameHumanCase).Count > 1)))
                {
            ...
            public override List<ForeignKey> ReadForeignKeys()
            {
                ...
                using (DbDataReader rdr = Cmd.ExecuteReader())
                {
                    while (rdr.Read())
                    {
                        ...
                        fkList.Add(new ForeignKey(fkTableName, fkSchema, pkTableName, pkSchema, fkColumn, pkColumn,
                            constraintName));
                    }
                }
            ...
            public override void ProcessForeignKeys(List<ForeignKey> fkList, Tables tables, bool useCamelCase,
                bool prependSchemaName, string collectionType, bool checkForFkNameClashes, bool includeComments)
            {
                ...
                foreach (var constraint in constraints)
                {
                    ...
                    var fkCols = foreignKeys.Select(x => fkTable.Columns.Find(n => n.PropertyName == x.FkColumn))
                        .Where(x => x != null).OrderBy(o => o.Ordinal).ToList();
                    var pkCols = foreignKeys.Select(x => pkTable.Columns.Find(n => n.PropertyName == x.PkColumn))
                        .Where(x => x != null).OrderBy(o => o.Ordinal).ToList();
                    ...
                    string pkPropName = fkTable.GetUniqueColumnPropertyName(pkTableHumanCase, foreignKey, useCamelCase,
                        checkForFkNameClashes, true);
                    bool fkMakePropNameSingular = (relationship == Relationship.OneToOne);
                    string fkPropName = pkTable.GetUniqueColumnPropertyName(fkTable.NameHumanCase, foreignKey, useCamelCase,
                        checkForFkNameClashes, fkMakePropNameSingular);

                    fkCol.EntityFk = null;
                    string.Format("public virtual {0} {1} {2}{3}", pkTableHumanCase, pkPropName, "{ get; set; }",
                        includeComments ? " // " + foreignKey.ConstraintName : string.Empty);

                    string manyToManyMapping;
                    if (foreignKeys.Count > 1)
                        manyToManyMapping = string.Format("c => new {{ {0} }}",
                            string.Join(", ", fkCols.Select(x => "c." + x.PropertyNameHumanCase).ToArray()));
                    else
                        manyToManyMapping = string.Format("c => c.{0}", fkCol.PropertyNameHumanCase);

                    fkCol.ConfigFk = string.Format("{0};{1}",
                        GetRelationship(relationship, fkCol, pkCol, pkPropName, fkPropName, manyToManyMapping),
                        includeComments ? " // " + foreignKey.ConstraintName : string.Empty);
                    pkTable.AddReverseNavigation(relationship, pkTableHumanCase, fkTable, fkPropName,
                        string.Format("{0}.{1}", fkTable.Name, foreignKey.ConstraintName), collectionType, includeComments);
                }
            }
            ...
            private static string GetRelationship(Relationship relationship, Column fkCol, Column pkCol, string pkPropName,
                string fkPropName, string manyToManyMapping)
            {
                return string.Format("Has{0}(a => a.{1}){2}", GetHasMethod(fkCol, pkCol), pkPropName,
                    GetWithMethod(relationship, fkCol, fkPropName, manyToManyMapping));
            }
                ...
                string manyToManyMapping)
            {
                switch (relationship)
                {
                    ...
                    case Relationship.ManyToOne:
                        return string.Format(".WithMany(b => b.{0}).HasForeignKey(c => c.{1})", fkPropName,
                            fkCol.PropertyNameHumanCase);
            ...
            private static Column CreateColumn(IDataRecord rdr, Regex rxClean, Table table, bool useCamelCase)
            {
                ...
                col.PropertyNameHumanCase =
                    (useCamelCase ? Inflector.ToTitleCase(col.PropertyName) : col.PropertyName).Replace(" ", "");
        ...
        public sealed class ForeignKey
        {
            ...
            public ForeignKey(string fkTableName, string fkSchema, string pkTableName, string pkSchema, string fkColumn,
                string pkColumn, string constraintName)
            {
            ...
            public string PkTableHumanCase(bool useCamelCase, bool prependSchemaName)
            {
                ...
                string pkTableHumanCase = (useCamelCase ? Inflector.ToTitleCase(singular) : singular).Replace(" ", "")
                    .Replace("$", "");
        ...
        public sealed class Table
        {
            ...
            public string GetUniqueColumnPropertyName(string tableNameHumanCase, ForeignKey foreignKey, bool useCamelCase,
                bool checkForFkNameClashes, bool makeSingular)
            {
            ...
            public void AddReverseNavigation(Relationship relationship, string fkName, Table fkTable, string propName,
                string constraint, string collectionType, bool includeComments)
            {

            }

            public void AddMappingConfiguration(ForeignKey left, ForeignKey right, bool useCamelCase,
                bool prependSchemaName)
            {
            ...
            public void IdentifyMappingTable(List<ForeignKey> fkList, Tables tables, bool useCamelCase,
                bool prependSchemaName, string collectionType, bool checkForFkNameClashes, bool includeComments)
            {
                IsMapping = false;
                ...
                rightTable.AddReverseNavigation(Relationship.ManyToMany, rightTable.NameHumanCase, leftTable,
                    rightTable.GetUniqueColumnPropertyName(leftTable.NameHumanCase, left, useCamelCase,
                        checkForFkNameClashes, false), null, collectionType, includeComments);

                leftTable.AddReverseNavigation(Relationship.ManyToMany, leftTable.NameHumanCase, rightTable,
                    leftTable.GetUniqueColumnPropertyName(rightTable.NameHumanCase, right, useCamelCase,
                        checkForFkNameClashes, false), null, collectionType, includeComments);
            }
        }
        ...
        public class Tables : List<Table>
        {
            ...
            public void IdentifyMappingTables(List<ForeignKey> fkList, bool useCamelCase, bool prependSchemaName,
                string collectionType, bool checkForFkNameClashes, bool includeComments)
            {
                foreach (var tbl in this.Where(x => x.HasForeignKey))
                {
                    tbl.IdentifyMappingTable(fkList, this, useCamelCase, prependSchemaName, collectionType,
                        checkForFkNameClashes, includeComments);
                }
            }
